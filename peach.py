...
def check_args():
    if (len(args) < expected_args) or (len(args) > expected_args):
        throw_err(f"got argument {str(len(args))} but expected {str(expected_args)}!")
        return False
    else:
        return True
...
def create_environment(name, location):
    ...
    if os.path.exists(location):
        ...
        creds.write('environment generated by peach') ; creds.close()
    else:
    ...
    if check_args() is True:
        ...
        if args[1].lower() in create_syntax:
            ...
            if args[2].lower() in languages:
                ...
                create_environment(folder_name, folder_loc)
            else:
